...
source "azure-arm" "ansible-runner" {
  ...
  temp_computer_name                     = var.az_build.temp_computer_name
  temp_nic_name                          = var.az_build.temp_nic_name
...
build {
  name    = "ansible-runner"
  sources = ["source.azure-arm.ansible-runner"]

  provisioner "ansible" {
    command              = "./command_ansible.sh"
    playbook_file        = "./ansible-remote-playbook.yml"
  ...
  provisioner "ansible-local" {
    ...
    inventory_file = "./ansible-local-inventory.ini"
  }
}